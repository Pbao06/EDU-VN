...
export function CareerDetail({
  ...
  return (
    <article className={`w-full max-w-5xl ${className}`}>
      {/* ============ 1. HERO — the one card ============ */}
      <section className="relative overflow-hidden rounded-[32px] border-[2.5px] border-black bg-white p-6 shadow-[10px_10px_0_0_#000] sm:p-9">
        ...
        <div className="relative flex flex-col gap-6 sm:flex-row sm:items-start sm:gap-7">
          ...
          <div className="relative shrink-0">
            ...
            >
              <Briefcase className="h-12 w-12 text-black sm:h-14 sm:w-14" strokeWidth={2.2} />
            </div>
          </div>
      ...
      {/* ============ 3. SALARY — big numbers, divider split, no card bg ============ */}
      <Section
        icon={<Wallet className="h-4 w-4" strokeWidth={2.5} />}
        title="Reference Salary"
      >
        <div className="grid grid-cols-1 divide-y-2 divide-black/10 sm:grid-cols-2 sm:divide-x-2 sm:divide-y-0">
          <div className="pb-5 sm:pb-0 sm:pr-8">
            <span className="text-[11px] font-extrabold uppercase tracking-widest text-black/50">
              Minimum
            </span>
            <div className="mt-1 flex items-baseline gap-1.5">
              <span className="text-3xl font-extrabold tracking-tight text-black sm:text-4xl">
                {formatSalary(minSalary, currency)}
              </span>
          ...
          <div className="pt-5 sm:pt-0 sm:pl-8">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-full border-2 border-black bg-[#FFD84D] px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-wider text-black">
                Top
              </span>
            </div>
            <div className="mt-1 flex items-baseline gap-1.5">
      ...
      {/* ============ 4. RESPONSIBILITIES — divided list, no per-item card ============ */}
      <Section
        ...
        <ul className="divide-y divide-black/10">
          {responsibilities.map((item, i) => (
            <li key={i} className="flex items-start gap-3 py-3.5 first:pt-0">
              <span
                className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border-2 border-black bg-[#7BE495]"
                aria-hidden
              >
                <Check className="h-3.5 w-3.5 text-black" strokeWidth={3.5} />
              </span>
              <span className="text-sm font-semibold leading-relaxed text-black/80 sm:text-base">
                {item}
              </span>
            </li>
          ))}
      ...
      {/* ============ 6. LEARNING PATH PREVIEW — the other card (signature block) ============ */}
      <Section
        icon={<Rocket className="h-4 w-4" strokeWidth={2.5} />}
        title="What you should learn"
      >
        <div className="rounded-[28px] border-[2.5px] border-black bg-[#FFF6C9] p-5 shadow-[8px_8px_0_0_#000] sm:p-6">
          <ol className="divide-y divide-black/10">
            {learningPathPreview.map((item, i) => (
              ...
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border-2 border-black bg-white text-sm font-extrabold">
                  {i + 1}
                </span>
      ...
      {(onPrimaryAction || onSecondaryAction) && (
        <section
          className={`mt-10 flex gap-3 border-t-2 border-black pt-8 ${
            onSecondaryAction ? "flex-col sm:flex-row" : "justify-center"
          }`}
        >
          {onPrimaryAction && (
            <button
              type="button"
              onClick={onPrimaryAction} // goi ham tu ngoai truyen vao
              // href={learningPathId ? `/learningpath/${learningPathId}` : "/learningpath"}
              className={`cursor-pointer inline-flex h-14 items-center justify-center gap-2 rounded-2xl border-[2.5px] border-black bg-[#4D7CFF] text-base font-extrabold uppercase tracking-wide text-white shadow-[6px_6px_0_0_#000] transition-all hover:-translate-y-0.5 hover:shadow-[8px_8px_0_0_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-[3px_3px_0_0_#000] ${
                onSecondaryAction ? "flex-1" : "px-8"
              }`}
            >
              {primaryLabel}
          ...
          {onSecondaryAction && (
            <button
              ...
              className="cursor-pointer inline-flex h-14 flex-1 items-center justify-center gap-2 rounded-2xl border-[2.5px] border-black bg-[#FFD84D] text-base font-extrabold uppercase tracking-wide text-black shadow-[6px_6px_0_0_#000] transition-all hover:-translate-y-0.5 hover:shadow-[8px_8px_0_0_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-[3px_3px_0_0_#000]"
            >
              {secondaryLabel}
            </button>
          )}
...
function Section({
  ...
  return (
    <section className="border-b-2 border-black/10 py-8 first-of-type:pt-8 last-of-type:border-b-0">
